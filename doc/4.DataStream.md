...
* 第二中，**通过  DataStream API  指定**：有时候我们不想在 SourceFunction 里生成 timestamp 或者 watermark，或者说使用的 SourceFunction 本身不支持（比如外部数据源连接器），我们还可以在使用 DataStream API 的时候指定，调用的 DataStream.assignTimestampsAndWatermarks 这个方法，能够接收不同的 timestamp 和 watermark 的生成器。生成器可以分为两类：
  ...
  * 第二类是根据一些在流处理数据流中遇到的一些特殊记录生成的；实现接口：AssignerWithPunctuatedWatermarks
  ...
  ![image-20201019151708496](F:\GitOsChina\myflink\image\4StreamExecutionEnvironment-6.png)
  ...
  ![img](https://ververica.cn/wp-content/uploads/2019/09/008-1024x474.png)
...
![image-20201019190251834](F:\GitOsChina\myflink\image\4StreamExecutionEnvironment-7.png)
...
![image-20201019190141995](F:\GitOsChina\myflink\image\4StreamExecutionEnvironment-8.png)
...
![image-20201019190523665](F:\GitOsChina\myflink\image\4StreamExecutionEnvironment-9.png)
...
![image-20201019191048837](F:\GitOsChina\myflink\image\4StreamExecutionEnvironment-10.png)
